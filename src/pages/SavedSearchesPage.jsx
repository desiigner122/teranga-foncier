...
const formatSearchParams = (params) => {
  if (!params) return 'Critéres non définis.';
  const parts = [];
  ...
  if (params.minArea) parts.push(`Surface min: ${params.minArea} mé`);
  if (params.maxArea) parts.push(`Surface max: ${params.maxArea} mé`);
  
  if (parts.length === 0) return "Tous les terrains (aucun critére spécifique).";
  return parts.join(' | ');
};
...
const SavedSearchCard = ({ search, onToggleNotify, onDelete, onEdit, onViewResults }) => (
  ...
  >
    <Card className="shadow-md hover:shadow-lg transition-shadow duration-300 bg-card border border-border/50">
      <CardHeader>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <CardTitle className="text-lg text-primary mb-1 sm:mb-0 flex items-center">
            ...
            {search.search_name || 'Recherche Sans Nom'}
          </CardTitle>
      ...
      <CardContent>
        <p className="text-sm font-medium mb-1.5 text-foreground">Critéres :</p>
             ...
             <AlertDialog>
                ...
                <AlertDialogContent>
                   <AlertDialogHeader>
                     ...
                     <AlertDialogDescription>
                       Voulez-vous vraiment supprimer la recherche sauvegardée "{search.search_name || 'Recherche Sans Nom'}" ? Cette action est irréversible et arrétera les notifications associées.
                     </AlertDialogDescription>
                   </AlertDialogHeader>
                   <AlertDialogFooter>
...
const SavedSearchesPage = () => {
  ...
  const handleDeleteSearch = (searchId) => {
    try {
      ...
      toast({ title: "Recherche Supprimée", description: "La recherche sauvegardée a été supprimée avec succés." });
    } catch (err) {
  ...
  const handleEditSearch = (search) => {
     toast({ title: "Fonctionnalité é venir", description: "La modification des recherches sera bientét disponible. Pour l'instant, supprimez et recréez votre recherche." });
  };
        ...
        description: (
          <div>
            ...
            <ol className="list-decimal list-inside text-sm space-y-1">
              ...
              <li>Cliquez sur le bouton "Sauvegarder la Recherche" (bientét disponible en haut des filtres).</li>
            </ol>
          </div>
        ),
        duration: 8000,
    ...
    >
      <div className="mb-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="text-center sm:text-left">
          ...
          <p className="text-muted-foreground mt-1">Gérez vos critéres de recherche et soyez alerté des nouvelles parcelles correspondantes.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-card rounded-lg border border-dashed border-border/70 shadow-sm">
          ...
          <p className="text-muted-foreground mb-6 max-w-md mx-auto">Sauvegardez vos critéres de recherche depuis la page des terrains pour étre alerté des nouvelles opportunités.</p>
