...
const SettingsPage = () => {
   ...
   const handleSaveChanges = async (e) => {
      ...
      toast({ 
         title: "Paramétres Sauvegardés", 
         description: "Vos préférences ont été mises é jour avec succés.",
      ...
      <form onSubmit={handleSaveChanges}>
         <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <Card className="mb-6 shadow-lg hover:shadow-primary/10 transition-shadow">
               ...
               <CardContent className="space-y-5">
                  {Object.entries({
                     emailNewParcel: "Nouvelles parcelles correspondant é mes recherches",
                     emailPriceChange: "Changements de prix sur mes parcelles favorites",
                     emailRequestUpdate: "Mises é jour sur mes demandes en cours",
                     appGeneralUpdates: "Annonces générales et mises é jour de la plateforme (via notifications internes)",
                  }).map(([key, label]) => (
            ...
            <Card className="mb-6 shadow-lg hover:shadow-primary/10 transition-shadow">
               ...
               <CardContent className="space-y-4">
                  <div className="grid gap-2">
                     <Label htmlFor="theme-select">Théme Visuel</Label>
                     <Select value={preferences.theme} onValueChange={(value) => handlePreferenceChange('theme', value)}>
                        <SelectTrigger id="theme-select" className="w-full sm:w-[200px]">
                           <SelectValue placeholder="Choisir un théme" />
                        </SelectTrigger>
                        <SelectContent>
                           <SelectItem value="light">Clair</SelectItem>
                           <SelectItem value="dark">Sombre</SelectItem>
                           <SelectItem value="system">Systéme</SelectItem>
                        </SelectContent>
                     </Select>
                  </div>
                   <div className="grid gap-2">
                     ...
                     <Select value={preferences.language} onValueChange={(value) => handlePreferenceChange('language', value)}>
                        ...
                        <SelectContent>
                           <SelectItem value="fr">Franéais</SelectItem>
            ...
            <Card className="mb-6 shadow-lg hover:shadow-primary/10 transition-shadow">
               ...
               <CardContent className="space-y-4">
                  ...
                  <div className="border p-3 rounded-md bg-muted/20">
                     <h4 className="font-medium text-sm mb-1">Authentification é Deux Facteurs (2FA)</h4>
                     <p className="text-xs text-muted-foreground mb-2">Renforcez la sécurité de votre compte.</p>
                     <Button variant="outline" size="sm" onClick={() => toast({ title: "2FA (Bientét disponible)", description: "L'authentification é deux facteurs sera bientét disponible."})}>Configurer 2FA (Bientét)</Button>
                  </div>
                  <div className="border p-3 rounded-md bg-muted/20">
         ...
         <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{delay: 0.3}} className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-6 border-t">
            <Button type="submit" size="lg" className="w-full sm:w-auto" disabled={isSaving}>
               <Save className="mr-2 h-4 w-4" /> {isSaving ? "Sauvegarde en cours..." : "Sauvegarder Tous les Paramétres"}
            </Button>
            <AlertDialog>
               ...
               <AlertDialogContent>
                  <AlertDialogHeader>
                     <AlertDialogTitle className="flex items-center"><AlertCircle className="text-destructive mr-2"/>étes-vous absolument sér ?</AlertDialogTitle>
                     <AlertDialogDescription>
