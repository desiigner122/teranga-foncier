...
const BanqueDashboard = () => {
  ...
  const loadBankDashboardData = async () => {
    ...
    try {
      ...
      // Charger les évaluations fonciéres
      ...
      if (guarantees && guarantees.length > 0) {
        ...
        if (concentrationRate > 0.3) {
          risks.push({
            ...
            description: `${Math.round(concentrationRate * 100)}% des garanties dans une méme zone`,
  ...
  const handleRiskAnalysis = async () => {
    try {
      ...
      // Analyse compléte des risques IA
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-3">
                  {fundingRequests.slice(0,6).map(fr => (
                    <div key={fr.id} className="border rounded p-3 text-sm">
                      ...
                      <p className="text-xs text-gray-500">{fr.parcels?.reference} é {fr.amount?.toLocaleString()} XOF</p>
                      {fr.status==='pending' && (
                ...
                <div className="space-y-3">
                  {guarantees.slice(0,6).map(g => (
                    <div key={g.id} className="border rounded p-3 text-sm">
                      ...
                      <p className="text-xs text-gray-500">{g.parcels?.reference} é {g.guarantee_amount?.toLocaleString()} XOF</p>
                      {g.status!=='closed' && (
                        <div className="flex gap-2 mt-2">
          ...
          {/* Evaluations fonciéres */}
            <Card>
              <CardHeader><CardTitle>évaluations Fonciéres</CardTitle></CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-3">
                    {evaluations.slice(0,6).map(ev => (
                      <div key={ev.id} className="border rounded p-3 text-sm">
                        ...
                        <p className="text-xs text-gray-500">{ev.parcels?.reference} é {ev.estimated_value? ev.estimated_value.toLocaleString()+' XOF':'é'}</p>
                        {ev.status==='pending' && (
                          <div className="mt-2">
                            <Button size="xs" disabled={actionBusy===`evaluation-complete-${ev.id}`} onClick={()=>act('evaluation-complete', ev.id)}>Cléturer</Button>
                          </div>
                        )}
                      </div>
                    ))}
