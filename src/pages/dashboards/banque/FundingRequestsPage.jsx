...
const { toast } = useToast();
  ...
  const loadFundingRequests = async () => {
    try {
      ...
      if (user && user.id) {
        // Utiliser la nouvelle méthode pour récupérer les demandes destinées spécifiquement é cette banque
  ...
  const handleRequestAction = async (requestId, newStatus, comment = '') => {
    try {
      ...
      setIsDetailModalOpen(false);
        ...
        variant: "destructive",
        title: "Erreur",
        description: "Impossible de mettre é jour la demande"
      });
    } finally {
