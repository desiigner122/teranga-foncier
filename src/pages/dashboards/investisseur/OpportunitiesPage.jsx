...
const OpportunitiesPage = () => {
  ...
  const { 
    ...
    error, 
    refetch 
  } = useRealtimeTable('market_predictions');
  ...
  const filtered = useMemo(() => {
    ...
    return predictions.filter(p => {
      ...
      if (minROI && p.predicted_price_per_sqm && Number(minROI) > p.predicted_price_per_sqm) return false;
      return true;
    });
  }, [predictions, locationFilter, typeFilter, minROI]);

  const uniqueLocations = useMemo(() => {
    if (!predictions) return [];
    return Array.from(new Set(predictions.map(p => p.location))).slice(0, 25);
  }, [predictions]);

  const uniqueTypes = useMemo(() => {
    if (!predictions) return [];
    return Array.from(new Set(predictions.map(p => p.property_type).filter(Boolean))).slice(0, 25);
  }, [predictions]);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Erreur de rafraîchissement' });
    } finally {
      setRefreshing(false);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap justify-between gap-4 items-center">
        ...
        <Button variant="outline" size="sm" onClick={handleRefresh} disabled={refreshing} className="flex items-center"><RefreshCw className={`h-4 w-4 mr-2 ${refreshing ? 'animate-spin' : ''}`}/>Rafraîchir</Button>
      </div>
