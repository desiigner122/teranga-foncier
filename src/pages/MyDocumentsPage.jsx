      ...
      <Card className="mb-8">
        ...
        <CardContent className="space-y-4">
          <div className="border-2 border-dashed rounded-md p-6 text-center">
            ...
            <p className="text-sm mb-2">Sélectionnez des documents é téléverser (PDF, JPG, PNG)</p>
            <Input type="file" multiple onChange={handleFileChange} />
            {files.length>0 && <p className="text-xs mt-2">{files.length} fichier(s) prét(s).</p>}
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {docs.map(d => (
                <div key={d.id} className="border rounded-md p-3 text-sm space-y-1 bg-card shadow-sm">
                  ...
                  <p className="text-xs text-muted-foreground">{(d.file_size/1024).toFixed(1)} KB é {d.mime_type}</p>
