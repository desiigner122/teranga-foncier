...
const AdminDashboardPage = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const parcelStatusMap = parcels.reduce((acc, p) => {
      ...
      const monthlySales = Object.keys(monthlySalesMap).map(m => ({ name: m, value: monthlySalesMap[m] }));
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 p-4 md:p-8">
      ...
      <p className="text-muted-foreground">Vue d'ensemble et statistiques clés de la plateforme.</p>
