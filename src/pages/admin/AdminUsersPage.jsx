            ...
            <div className="md:col-span-1">
                <Card>
                    ...
                    <CardContent className="flex flex-col space-y-2">
                        ...
                        <TabButton value="all" label="Tous les utilisateurs" icon={Users} count={users.length} />
                    </CardContent>
                </Card>
            </div>
                                    ...
                                    {loading ? (
                                        ...
                                        <TableRow key={user.id}>
                                            ...
                                            <TableCell className="text-right">
                                                <Button variant="ghost" size="sm" onClick={() => openUserModal(user)}><Eye className="mr-2 h-4 w-4"/>Détails</Button>
                                            </TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                ...
                <div>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <img src={selectedUser.id_card_front_url} alt="Recto CNI" className="rounded-lg border mt-1 w-full" />
                        </div>
                        <div>
                            <Label>Verso de la carte d'identité</Label>
                            <img src={selectedUser.id_card_back_url} alt="Verso CNI" className="rounded-lg border mt-1 w-full" />
                        </div>
                    </div>
                </div>
                <div>
                    <h3 className="font-semibold mb-2">Assistant IA</h3>
                    <Button onClick={() => handleAnalyseWithAI(selectedUser)} disabled={isAiLoading} className="w-full">
                        {isAiLoading ? <LoadingSpinner size="small" className="mr-2"/> : <Sparkles className="mr-2 h-4 w-4" />}
                        {isAiLoading ? "Analyse en cours..." : "Analyser les documents avec l'IA"}
                    </Button>
                    {aiAnalysis && (
                        <Card className="mt-4">
                            <CardHeader><CardTitle className="text-base">Résultat de l'analyse</CardTitle></CardHeader>
                            <CardContent className="text-sm space-y-2 bg-muted p-4 rounded-lg">
                                {aiAnalysis.error ? <p className="text-red-500">{aiAnalysis.error}</p> :
                                <pre className="whitespace-pre-wrap font-sans">{JSON.stringify(aiAnalysis, null, 2)}</pre>}
                            </CardContent>
                        </Card>
                    )}
                </div>
