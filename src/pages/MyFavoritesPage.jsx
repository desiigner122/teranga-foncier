    ...
    <Card className="h-full flex flex-col overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300 relative group">
      ...
      <CardContent className="p-4 flex-grow">
        ...
        <div className="text-sm text-muted-foreground space-y-1">
          ...
          <p className="flex items-center"><Maximize className="h-4 w-4 mr-2 flex-shrink-0"/> Surface: <span className="font-medium text-foreground ml-1">{parcel.area_sqm ? `${parcel.area_sqm} mé` : 'N/A'}</span></p>
        </div>
      </CardContent>
...
const MyFavoritesPage = () => {
  ...
  const handleRemoveFavorite = async (parcelIdToRemove) => {
     ...
     try {
        ...
        if (success) {
          // Mettre é jour l'état local pour un feedback immédiat
    ...
    >
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-muted/50 rounded-lg border border-dashed">
          ...
          <p className="text-muted-foreground mb-6">Cliquez sur l'icéne ?? sur une parcelle pour l'ajouter ici.</p>
          <Button asChild>
