  ...
  const renderPaymentDetails = () => {
    switch (selectedMethod) {
      ...
      case 'check':
        return (
          <div className="space-y-4">
            <Input placeholder="Numéro du chéque" required />
            <Input placeholder="Banque émettrice" required />
            <p className="text-xs text-muted-foreground p-3 bg-muted/50 rounded-md">
              Le chéque doit étre libellé é l'ordre de "Teranga Foncier SA" et déposé é notre agence principale.
            </p>
          </div>
        );
      default:
  ...
  if (isPaid) {
    return (
      ...
      >
        ...
        <p className="text-lg text-muted-foreground mb-4">Votre transaction a été traitée avec succés (statut: {currentStatus || 'paid'}).</p>
        <p className="text-sm">Vous allez étre redirigé vers vos transactions...</p>
      </motion.div>
    );
  }
      ...
      <Card className="overflow-hidden">
        ...
        <CardContent className="pt-6">
          ...
          <div className="text-center mb-6">
            <p className="text-sm text-muted-foreground">Montant é payer</p>
