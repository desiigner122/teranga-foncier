...
const SecureMessagingPage = () => {
  ...
  useEffect(() => {
    if (!user) {
      setError("Veuillez vous connecter pour accéder é la messagerie.");
    } else {
      setError(null);
    }
  }, [user]);
              ...
              <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-muted/20">
                {loadingMessages ? (
                  <div className="space-y-4">
                   ...
                   <p className="text-center text-muted-foreground pt-10">Aucun message. Soyez le premier é en envoyer un !</p>
                ) : (
