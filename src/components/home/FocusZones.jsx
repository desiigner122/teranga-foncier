...
const zones = [
  ...
  { name: "Diamniadio", description: "Ville nouvelle en plein essor, pôle urbain moderne.", imageDesc: "Bâtiments futuristes Diamniadio Sénégal", link: "/parcelles?zone=Diamniadio" },
            ...
            <motion.div key={index} variants={itemVariants}>
              <Link to={zone.link}>
                <Card className="overflow-hidden h-full group relative rounded-xl border shadow-sm hover:shadow-lg transition-all duration-300">
                  <div className="aspect-[4/3] bg-muted relative"> {/* Added relative */}
                     <img 
                  ...
                  <CardContent className="absolute bottom-0 left-0 right-0 p-4 text-white z-20"> {/* Ensure z-index is higher */}
                    <h3 className="text-xl font-semibold mb-1 flex items-center">
                       <MapPin className="h-5 w-5 mr-2 text-accent_brand"/> {zone.name}
                    </h3>
                    <p className="text-sm text-gray-100 line-clamp-2 mb-2">{zone.description}</p> {/* Lighter text */}
